...
fn spend_in_fork_and_compact() {
	...
	let tx1 = build::transaction(
		vec![
			build::coinbase_input(consensus::reward(6, 0), key_id2.clone()),
			build::output(consensus::reward(6, 0) - 20000, key_id30.clone()),
	...
	let tx2 = build::transaction(
		vec![
			build::input(consensus::reward(8, 0) - 20000, key_id30.clone()),
			build::output(consensus::reward(8, 0) - 40000, key_id31.clone()),
			build::with_fee(20000),
		],
		&kc,
	)
	...
	K: Keychain,
{
	...
	let reward = libtx::reward::output(kc, &key_id, fees, 2).unwrap();
