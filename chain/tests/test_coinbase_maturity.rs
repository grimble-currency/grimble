...
fn test_coinbase_maturity() {
	...
	let reward = libtx::reward::output(&keychain, &key_id1, 0, 1).unwrap();
	...
	let amount = consensus::reward(1, 0);
	...
	let reward = libtx::reward::output(&keychain, &key_id3, fees, 2).unwrap();
	...
	for h in 0..3 {
		...
		let reward = libtx::reward::output(&keychain, &pk, 0, 3 + h).unwrap();
		let mut block = core::core::Block::new(&prev, vec![], Difficulty::min(), reward).unwrap();
		let next_header_info = consensus::next_difficulty(1, chain.difficulty_iter());
		block.header.timestamp = prev.timestamp + Duration::seconds(60);
		block.header.pow.secondary_scaling = next_header_info.secondary_scaling;
	...
	let fees = txs.iter().map(|tx| tx.fee()).sum();
	let next_header_info = consensus::next_difficulty(1, chain.difficulty_iter());
	let reward = libtx::reward::output(&keychain, &key_id4, fees, 6).unwrap();
	let mut block = core::core::Block::new(&prev, txs, Difficulty::min(), reward).unwrap();
