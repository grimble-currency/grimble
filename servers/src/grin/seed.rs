...
const MAINNET_DNS_SEEDS: &'static [&'static str] = &["mainnet.seed.grimblecurrency.com"];
const FLOONET_DNS_SEEDS: &'static [&'static str] = &["floonet.seed.grimblecurrency.com"];
		...
		for dns_seed in net_seeds {
			...
			match (dns_seed.to_owned(), 0).to_socket_addrs() {
				Ok(addrs) => addresses.append(
					&mut (addrs
						.map(|mut addr| {
							addr.set_port(if global::is_floonet() { 24414 } else { 4414 });
							addr
						})
